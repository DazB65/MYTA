<template>
  <div ref="containerRef" class="lazy-component">
    ...
    <div v-else-if="error" class="error-state">
      ...
      <button class="retry-button" @click="loadComponent">Try Again</button>
    </div>

    <!-- Loaded Component -->
    <Suspense v-else>
      <component :is="loadedComponent" v-bind="componentProps" @error="handleComponentError" />
      <template #fallback>
...
const props = defineProps({
  ...
  priority: {
    type: String,
    default: 'normal',
    validator: v => ['high', 'normal', 'low'].includes(v),
  },
...
const loadComponent = async () => {
  ...
  try {
    ...
    if (!componentDefinition)
      throw new Error('Component loader must return a valid component definition')
