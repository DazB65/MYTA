#!/bin/bash
# Test script for CreatorMate Docker setup
...
test_dockerfiles() {
    ...
    if docker build -f backend/Dockerfile --target production -t creatormate-backend-test backend/ --dry-run > /dev/null 2>&1; then
    ...
    if docker build -f frontend-new/Dockerfile --target production -t creatormate-frontend-test frontend-new/ --dry-run > /dev/null 2>&1; then
...
# Main test execution
main() {
    echo "🐳 CreatorMate Docker Setup Test"
    echo "================================="
    echo ""
