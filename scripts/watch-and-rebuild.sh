...
if command -v fswatch >/dev/null 2>&1; then
    ...
    fswatch -o frontend-vidalytics/src | while read; do rebuild; done
elif command -v inotifywait >/dev/null 2>&1; then
    echo "Using inotifywait to monitor changes..."
    while inotifywait -r -e modify,create,delete frontend-vidalytics/src; do rebuild; done
else
    echo "⚠️  No file watcher available. Install fswatch: brew install fswatch"
    echo "   Or manually run: cd frontend-vidalytics && npm run build"
fi